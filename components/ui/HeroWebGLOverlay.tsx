...
import type { RippleEffect, RippleEffectState } from '@/lib/interactive-hero/effects/webgl/ripple';
import type { SweepEffect, SweepEffectState } from '@/lib/interactive-hero/effects/webgl/sweep';
import type { VignetteEffect, VignetteEffectState } from '@/lib/interactive-hero/effects/webgl/vignette';
...
export const HeroWebGLOverlay = forwardRef<HeroWebGLOverlayRef, HeroWebGLOverlayProps>(
  function HeroWebGLOverlay({ containerRef, visible, intensity, className }, ref) {
    const canvasRef = useRef<HTMLDivElement>(null);
    const appRef = useRef<Application | null>(null);
    const effectsContainerRef = useRef<Container | null>(null);
    const activeEffectsRef = useRef<Map<string, { effect: WebGLEffect; graphics: Graphics }>>(new Map());
      ...
      activeEffectsRef.current.forEach(({ effect, graphics }) => {
        ...
        switch (effect.type) {
          case 'ripple': {
            ...
            const radius = rippleState.radius * Math.max(bounds.width, bounds.height);
    ...
    const addEffect = useCallback((effect: WebGLEffect) => {
    ...
    useEffect(() => {
      if (visible && activeEffectsRef.current.size > 0 && rafIdRef.current === null) {
        rafIdRef.current = requestAnimationFrame(renderEffects);
