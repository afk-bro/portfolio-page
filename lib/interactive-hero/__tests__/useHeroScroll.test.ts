...
describe('useHeroScroll', () => {
  ...
  it('cleans up on unmount', () => {
    ...
    ScrollTrigger.create.mockReturnValue({ kill: mockKill, progress: 0, direction: 1, getVelocity: jest.fn() });

    const { unmount } = renderHook(() => useHeroScroll({
      containerRef: mockContainerRef,
      enabled: true,
    }));
